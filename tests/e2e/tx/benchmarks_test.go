...
func BenchmarkTx(t *testing.B) {
	s := NewE2EBenchmarkSuite(t)
	t.Cleanup(s.Close)
	val := s.network.Validators[0]
	txBuilder := mkTxBuilder(t, s)
	...
	for i := 0; i < t.N; i++ {
		...
		for _, tc := range testCases {
			...
			assert.Equal(t, 12, len(res.GetResult().GetEvents()))
			assert.Assert(t, res.GetGasInfo().GetGasUsed() > 0) // Gas used sometimes change, just check it's not empty.
		}
	}
}

func NewE2EBenchmarkSuite(t testing.TB) *E2EBenchmarkSuite {
	...
	val := s.network.Validators[0]
	assert.NilError(t, s.network.WaitForNextBlock())
	...
	assert.NilError(t, err)
	assert.NilError(t, val.ClientCtx.Codec.UnmarshalJSON(out.Bytes(), &s.txRes))
	assert.Equal(t, uint32(0), s.txRes.Code, s.txRes)
	...
	assert.NilError(t, val.ClientCtx.Codec.UnmarshalJSON(out.Bytes(), &tr))
	assert.Equal(t, uint32(0), tr.Code)
...
func mkTxBuilder(t testing.TB, s *E2EBenchmarkSuite) client.TxBuilder {
	val := s.network.Validators[0]
	assert.NilError(t, s.network.WaitForNextBlock())
